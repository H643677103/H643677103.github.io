// 第一次播放音乐
var chyt_musicFirst = false;
// 快捷键
var chyt_keyboard = null;
// 音乐播放状态
var chyt_musicPlaying = false;
var $bodyWrap = document.getElementById("body-wrap");
var chyt_intype = false;
var chyt_keyUpEvent_timeoutId = null;
var chyt_keyUpShiftDelayEvent_timeoutId = null;
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const sidebarFn = {
    open: () => {
      chyt.sidebarPaddingR();
      chyt.animateIn(document.getElementById("menu-mask"), "to_show 0.5s");
    ...
    close: () => {
      ...
      chyt.animateOut(document.getElementById("menu-mask"), "to_hide 0.5s");
  ...
  const scrollDownInIndex = () => {
    const handleScrollToDest = () => {
      ...
      if (bbTimeList) {
        chyt.scrollToDest(bbTimeList.offsetTop - 62, 300);
      } else {
        chyt.scrollToDest(document.getElementById("home_top").offsetTop - 60, 300);
      }
    };

    const $scrollDownEle = document.getElementById("scroll-down");
    $scrollDownEle && chyt.addEventListenerPjax($scrollDownEle, "click", handleScrollToDest);
  };
  ...
  const addHighlightTool = function () {
    ...
    if (isHighlightShrink !== undefined) {
      highlightShrinkEle = `<i class="chytfont chyt-icon-angle-down expand ${highlightShrinkClass}"></i>`;
    }

    if (isHighlightCopy) {
      highlightCopyEle = '<div class="copy-notice"></div><i class="chytfont chyt-icon-paste copy-button"></i>';
    }
    ...
    const copy = (text, ctx) => {
      if (document.queryCommandSupported && document.queryCommandSupported("copy")) {
        ...
        if (GLOBAL_CONFIG.Snackbar !== undefined) {
          chyt.snackbarShow(GLOBAL_CONFIG.copy.success);
        } else {
        ...
        if (GLOBAL_CONFIG.Snackbar !== undefined) {
          chyt.snackbarShow(GLOBAL_CONFIG.copy.noSupport);
        } else {
    ...
    const highlightShrinkFn = ele => {
      ...
      if (chyt.isHidden($nextEle[$nextEle.length - 1])) {
        $nextEle.forEach(e => {
    ...
    const createEle = (lang, item, service) => {
      ...
      if (isShowTool) {
        ...
        chyt.addEventListenerPjax(hlTools, "click", highlightToolsFn);
      ...
      if (highlightHeightLimit && item.offsetHeight > highlightHeightLimit + 30) {
        ...
        ele.innerHTML = '<i class="chytfont chyt-icon-angle-double-down"></i>';
        chyt.addEventListenerPjax(ele, "click", expandCode);
    ...
    if (isHighlightLang) {
      if (isPrismjs) {
        $figureHighlight.forEach(function (item) {
          ...
          chyt.wrap(item, "figure", { class: "highlight" });
   ...
   */
  const runLightbox = () => {
    chyt.loadLightbox(document.querySelectorAll("#article-container img:not(.no-lightbox)"));
  };
  ...
  const runJustifiedGallery = function (ele) {
    ...
    const runJustifiedGallery = (item, arr) => {
      ...
      if (!item.classList.contains("lazyload") || arr.length < limit) {
        ...
        if (!item.classList.contains("btn_album_detail_lazyload") || item.classList.contains("page_img_lazyload")) {
          ...
          const clickBtnFn = () => {
            ...
            chyt.loadLightbox(item.querySelectorAll("img"));
      ...
      chyt.initJustifiedGallery(item);
      chyt.loadLightbox(item.querySelectorAll("img"));
      window.lazyLoadInstance && window.lazyLoadInstance.update();
    };
  ...
  const scrollFn = function () {
    ...
    function percentageScrollFn(currentTop) {
      ...
      // 获取页面的实际高度和视口高度
      let docHeight = $bodyWrap ? $bodyWrap.clientHeight : document.documentElement.scrollHeight;
      const winHeight = window.innerHeight || document.documentElement.clientHeight;
    
      // 计算可滚动的高度
      const contentMath = Math.max(docHeight - winHeight, 1);
      ...
      // 确保百分比在 1 到 100 之间
      const percentage = Math.max(1, Math.min(scrollPercentRounded, 100));
      ...
      function isInViewPortOfOneNoDis(el) {
        ...
        const offsetTop = el.getBoundingClientRect().top; // 使用 getBoundingClientRect 获取元素相对视口的位置
    ...
    const scrollTask = chyt.throttle(() => {
    ...
    scrollTask();
    chyt.addEventListenerPjax(window, "scroll", scrollTask, { passive: true });
  };
  ...
  const scrollFnToDo = function () {
    ...
    if (isToc) {
      ...
      const tocItemClickFn = e => {
        ...
        e.preventDefault();
        chyt.scrollToDest(
          chyt.getEleTop(document.getElementById(decodeURI(target.getAttribute("href")).replace("#", ""))),
          300
        );
      ...
      chyt.addEventListenerPjax($cardToc, "click", tocItemClickFn);
    ...
    const findHeadPosition = function (top) {
      ...
      if (isAnchor) chyt.updateAnchor(currentId);
    ...
    // main of scroll
    const tocScrollFn = chyt.throttle(() => {
      const currentTop = window.scrollY || document.documentElement.scrollTop;
      findHeadPosition(currentTop);
    }, 100);

    chyt.addEventListenerPjax(window, "scroll", tocScrollFn, { passive: true });
  };
  ...
  const handleThemeChange = mode => {
    ...
    if (!GLOBAL_CONFIG_SITE.isPost) {
      ...
      root.style.setProperty("--chyt-bar-background", "var(--chyt-meta-theme-color)");
      requestAnimationFrame(() => {
        chyt.initThemeColor();
      });
      ...
      document.documentElement.style.setProperty(
        "--chyt-main",
        getComputedStyle(document.documentElement).getPropertyValue("--chyt-theme")
      );
      document.documentElement.style.setProperty(
        "--chyt-theme-op",
        getComputedStyle(document.documentElement).getPropertyValue("--chyt-main") + "23"
      );
      document.documentElement.style.setProperty(
        "--chyt-theme-op-deep",
        getComputedStyle(document.documentElement).getPropertyValue("--chyt-main") + "dd"
      );
    }
  };
  ...
  const rightSideFn = {
    ...
    darkmode: () => {
      ...
      if (willChangeMode === "dark") {
        activateDarkMode();
        GLOBAL_CONFIG.Snackbar !== undefined && chyt.snackbarShow(GLOBAL_CONFIG.Snackbar.day_to_night);
      } else {
        activateLightMode();
        GLOBAL_CONFIG.Snackbar !== undefined && chyt.snackbarShow(GLOBAL_CONFIG.Snackbar.night_to_day);
      }
  ...
  const openMobileMenu = () => {
    ...
    chyt.addEventListenerPjax(document.getElementById("toggle-menu"), "click", handleClick);
  };
  ...
  const addCopyright = () => {
    ...
    const handleCopy = e => {
      if (copy) {
        chyt.snackbarShow(languages.copySuccess);
      }
      if (copyrightEbable) {
        e.preventDefault();
  ...
  const addRuntime = () => {
    ...
    if ($runtimeCount) {
      ...
      $runtimeCount.textContent = `${chyt.diffDate(publishDate)} ${GLOBAL_CONFIG.runtime}`;
    }
  };
  ...
  const addLastPushDate = () => {
    ...
    if ($lastPushDateItem) {
      ...
      $lastPushDateItem.textContent = chyt.diffDate(lastPushDate, true);
    }
  };
  ...
  const addTableWrap = () => {
    ...
    $table.forEach(item => {
      if (!item.closest(".highlight")) {
        chyt.wrap(item, "div", { class: "table-wrap" });
      }
    });
  };
  ...
  const tabsFn = () => {
      ...
      const navClickHandler = function (e) {
        ...
        if (isJustifiedGallery) {
          ...
          if ($isTabJustifiedGallery.length > 0) {
            chyt.initJustifiedGallery($isTabJustifiedGallery);
          }
        }
      };
      chyt.addEventListenerPjax(item.firstElementChild, "click", navClickHandler);
    };
    ...
    const addTabToTopEventListener = item => {
      const btnClickHandler = e => {
        const target = e.target.closest("button");
        if (!target) return;
        chyt.scrollToDest(chyt.getEleTop(item), 300);
      };
      chyt.addEventListenerPjax(item.lastElementChild, "click", btnClickHandler);
    };
  ...
  const toggleCardCategory = () => {
    ...
    chyt.addEventListenerPjax(cardCategory, "click", handleToggleBtn, true);
  };
  ...
  const switchComments = () => {
    ...
    chyt.addEventListenerPjax(switchBtn, "click", handleSwitchBtn);
  };

  const addPostOutdateNotice = function () {
    const data = GLOBAL_CONFIG.noticeOutdate;
    const diffDay = chyt.diffDate(GLOBAL_CONFIG_SITE.postUpdate);
  ...
  const relativeDate = function (selector) {
    selector.forEach(item => {
      ...
      item.textContent = chyt.diffDate(timeVal, true);
  ...
  const coverColor = async () => {
    ...
    if (GLOBAL_CONFIG.mainTone) {
      if (GLOBAL_CONFIG_SITE.postMainColor) {
        ...
        root.style.setProperty("--chyt-bar-background", value);
        requestAnimationFrame(() => {
          chyt.initThemeColor();
        });

        if (GLOBAL_CONFIG.mainTone.cover_change) {
          document.documentElement.style.setProperty("--chyt-main", value);
          document.documentElement.style.setProperty(
            "--chyt-theme-op",
            getComputedStyle(document.documentElement).getPropertyValue("--chyt-main") + "23"
          );
        ...
        const fallbackValue = "var(--chyt-theme)";
        let fetchPath = "";
            ...
            if (GLOBAL_CONFIG.mainTone.mode == "both") {
              ...
              try {
                ...
                if (response.ok && response.headers.get("content-type")?.includes("application/json")) {
                  ...
                  root.style.setProperty("--chyt-bar-background", fallbackValue);
                  requestAnimationFrame(() => {
                    chyt.initThemeColor();
                  });
                  document.documentElement.style.setProperty("--chyt-main", fallbackValue);
                }
              } catch {
                root.style.setProperty("--chyt-bar-background", fallbackValue);
                requestAnimationFrame(() => {
  ...
  const listenNavMusicPause = function () {
    const timer = setInterval(() => {
      if (navMusicEl && navMusicEl.querySelector("#nav-music meting-js").aplayer) {
        clearInterval(timer);
        let msgPlay = '<i class="chytfont chyt-icon-play"></i><span>播放音乐</span>';
        let msgPause = '<i class="chytfont chyt-icon-pause"></i><span>暂停音乐</span>';
        navMusicEl.querySelector("#nav-music meting-js").aplayer.on("pause", function () {
          navMusicEl.classList.remove("playing");
          document.getElementById("menu-music-toggle").innerHTML = msgPlay;
          document.getElementById("nav-music-hoverTips").innerHTML = "音乐已暂停";
          document.querySelector("#consoleMusic").classList.remove("on");
          chyt_musicPlaying = false;
  ...
  window.onkeydown = function (e) {
    123 === e.keyCode && chyt.snackbarShow("开发者模式已打开，请遵循GPL协议", !1);
  };
  ...
  function executeShortcutKeyFunction() {
    // 是否开启快捷键
    chyt_keyboard = localStorage.getItem("keyboardToggle") ? localStorage.getItem("keyboardToggle") : false;
    ...
    function keyDownEvent(event) {
      ...
      const isKeyboardEnabled = chyt_keyboard;
      const isInInputField = chyt_intype;
      ...
      if (isKeyboardEnabled && isShiftKeyPressed && !isInInputField) {
        chyt_keyUpShiftDelayEvent_timeoutId = setTimeout(() => {
          switch (event.keyCode) {
            case 16:
              chyt_keyUpEvent_timeoutId = setTimeout(() => {
    ...
    function keyUpEvent(event) {
      chyt_keyUpEvent_timeoutId && clearTimeout(chyt_keyUpEvent_timeoutId);
      chyt_keyUpShiftDelayEvent_timeoutId && clearTimeout(chyt_keyUpShiftDelayEvent_timeoutId);
      if (event.keyCode === 16) {
  ...
  const unRefreshFn = function () {
    window.addEventListener("resize", () => {
      adjustMenu(false);
      mobileSidebarOpen && chyt.isHidden(document.getElementById("toggle-menu")) && sidebarFn.close();
    });
  ...
  window.refreshFn = function () {
    ...
    chyt.initIndexEssay();
    chyt.changeTimeInEssay();
    chyt.removeBodyPaceClass();
    chyt.qrcodeCreate();
    chyt.changeTimeInAlbumDetail();
    chyt.reflashEssayWaterFall();
    chyt.sayhi();
    chyt.stopImgRightDrag();
    chyt.addNavBackgroundInit();
    chyt.setValueToBodyType();
    chyt.catalogActive();
    chyt.tagsPageActive();
    chyt.categoriesBarActive();
    chyt.topCategoriesBarScroll();
    chyt.switchRightClickMenuHotReview();
    chyt.getCustomPlayList();
    chyt.addEventListenerConsoleMusicList(false);
    chyt.initPaginationObserver();
