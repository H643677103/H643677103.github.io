const chyt = {
  ...
  snackbarShow: (text, showActionFunction = false, duration = 2000, actionText = false) => {
    ...
    root.style.setProperty("--chyt-snackbar-time", duration + "ms");
    ...
    if (service === "fancybox") {
      Array.from(ele).forEach(i => {
        if (i.parentNode.tagName !== "A") {
          ...
          const dataSrc1600 = dataSrc + '_1600w';
          const dataCaption = i.title || i.alt || "";
          if (dataSrc.indexOf('!blogimg') != -1) {
            chyt.wrap(i, "a", {
              href: dataSrc,
              "data-fancybox": "images",
              "data-caption": dataCaption,
              "data-thumb": dataSrc,
              "data-srcset": `${dataSrc1600} 1600w`
            });
          } else {
            chyt.wrap(i, "a", {
              href: dataSrc,
  ...
  initThemeColor: function () {
    let themeColor = getComputedStyle(document.documentElement)
      .getPropertyValue("--chyt-bar-background")
      .trim()
      .replace('"', "")
      .replace('"', "");
    ...
    if (currentTop > 26) {
      if (chyt.is_Post()) {
        themeColor = getComputedStyle(document.documentElement)
          .getPropertyValue("--chyt-meta-theme-post-color")
          .trim()
          .replace('"', "")
          .replace('"', "");
      }
  ...
  downloadImage: function (imgsrc, name) {
    ...
    if (rm.downloadimging == false) {
      ...
      chyt.snackbarShow("正在下载中，请稍后", false, 10000);
      setTimeout(function () {
        let image = new Image();
        // 解决跨域 Canvas 污染问题
        ...
        image.src = imgsrc;
        chyt.snackbarShow("图片已添加盲水印，请遵守版权协议");
        rm.downloadimging = false;
      }, "10000");
    } else {
      chyt.snackbarShow("有正在进行中的下载，请稍后再试");
    }
  },
  //禁止图片右键单击
  ...
  catalogActive: function () {
    ...
    if ($list) {
      ...
      chyt.scrollByMouseWheel($list, $catalog);
    }
  },
  // Page Tag 激活
  tagsPageActive: function () {
    ...
    if ($list) {
      ...
      chyt.scrollByMouseWheel($list, $tagPageTags);
    }
  },
  // 修改时间显示"最近"
  ...
  changeTimeInEssay: function () {
    document.querySelector("#bber") &&
      document.querySelectorAll("#bber time").forEach(function (e) {
        ...
        (t.innerText = chyt.diffDate(datetime, true)), (t.style.display = "inline");
      });
  },
  // 修改相册集中的时间
  ...
  getCustomPlayList: function () {
    ...
    const userId = "8831719294";
    const userServer = "netease";
    const anMusicPageMeting = document.getElementById("anMusic-page-meting");
    if (urlParams.get("id") && urlParams.get("server")) {
      const id = urlParams.get("id");
      const server = urlParams.get("server");
      anMusicPageMeting.innerHTML = `<meting-js id="${id}" server=${server} type="playlist" type="playlist" mutex="true" preload="auto" theme="var(--chyt-main)" order="list" list-max-height="calc(100vh - 190px)!important"></meting-js>`;
    } else {
      anMusicPageMeting.innerHTML = `<meting-js id="${userId}" server="${userServer}" type="playlist" mutex="true" preload="auto" theme="var(--chyt-main)" order="list" list-max-height="calc(100vh - 190px)!important"></meting-js>`;
    }
    chyt.changeMusicBg(false);
  },
  //隐藏今日推荐
  ...
  addEventListenerMusic: function () {
    ...
    anMusicBtnGetSong.addEventListener("click", () => {
      if (changeMusicListFlag) {
        ...
        chyt.cacheAndPlayMusic();
      }
    });
    anMusicRefreshBtn.addEventListener("click", () => {
      localStorage.removeItem("musicData");
      chyt.snackbarShow("已移除相关缓存歌曲");
    });
    anMusicSwitchingBtn.addEventListener("click", () => {
      chyt.changeMusicList();
    });
  ...
  musicBindEvent: function () {
    document.querySelector("#nav-music .aplayer-music").addEventListener("click", function () {
      chyt.musicTelescopic();
    });
    document.querySelector("#nav-music .aplayer-button").addEventListener("click", function () {
      chyt.musicToggle(false);
    });
  },
  ...
  keyboardToggle: function () {
    const isKeyboardOn = chyt_keyboard;
  ...
  scrollCategoryBarToRight: function () {
    ...
    if (items && nextButton) {
      ...
      if (items.scrollLeft + items.clientWidth + 1 >= items.scrollWidth) {
        ...
        nextButton.innerHTML = '<i class="chytfont chyt-icon-angle-double-right"></i>';
      } else {
        // 滚动到下一个视图
        items.scrollBy({
          left: itemsWidth,
