  ...
  function popCommentBarrage(data) {
    ...
    barrage.innerHTML = `
          <div class="barrageHead">
            ...
            }" href="javascript:chyt.scrollTo('#post-comment')"">
            ...
            <a class="comment-barrage-close" href="javascript:chyt.switchCommentBarrage()"><i class="chytfont chyt-icon-xmark"></i></a>
          </div>
          <chyt class="barrageContent" onClick="window.location.hash = '${data.id}'">
            ${data.comment}
          </chyt>
        `;

    // 获取chyt标签内的所有pre元素
    let chytPres = barrage.querySelectorAll("chyt pre");

    // 遍历每个pre元素，将其替换为"【代码】"
    chytPres.forEach(pre => {
    ...
    // 获取chyt标签内的所有图片元素
    let chytImages = barrage.querySelectorAll("chyt img");

    // 遍历每个图片元素，将其替换为"【图片】"，但排除带有class=tk-owo-emotion的图片
    chytImages.forEach(image => {
