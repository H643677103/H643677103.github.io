(function () {
  ...
  const aiTitleRefreshIcon = post_ai.querySelector(".ai-title .chytfont.chyt-icon-arrow-rotate-right");
  let aiReadAloudIcon = post_ai.querySelector(".chyt-icon-circle-dot");
  ...
  async function readAloud() {
    if (!summaryID) {
      chyt.snackbarShow("摘要还没加载完呢，请稍后。。。");
      return;
    }
    aiReadAloudIcon = post_ai.querySelector(".chyt-icon-circle-dot");
    ...
    try {
      ...
      if (response.status === 403) {
        ...
        const audioBlog = await response.blog();
