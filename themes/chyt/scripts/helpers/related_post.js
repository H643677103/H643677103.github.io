...
hexo.extend.helper.register("related_posts", function (currentPost, allPosts) {
  ...
  if (relatedPosts.length > 0) {
    ...
    result += `<div class="headline"><i class="chytfont chyt-icon-thumbs-up fa-fw" style="font-size: 1.5rem; margin-right: 4px"></i><span>${headlineLang}</span></div>`;
    ...
    for (let i = 0; i < Math.min(relatedPosts.length, limitNum); i++) {
      ...
      if (dateType === "created") {
        result += `<div class="content is-center"><div class="date"><i class="chytfont chyt-icon-calendar-days fa-fw"></i> ${this.date(
          relatedPosts[i].created,
          hexoConfig.date_format
        )}</div>`;
      } else {
        result += `<div class="content is-center"><div class="date"><i class="chytfont chyt-icon-history fa-fw"></i> ${this.date(
